...
constexpr Color COLOR_WHITE = { 31, 31, 31 };
constexpr Color COLOR_RED = { 31, 0, 0 };
constexpr Color COLOR_ORANGE = { 31, 16, 1 };
constexpr Color COLOR_GREEN = { 0, 31, 0 };
constexpr Color COLOR_BLUE = { 0, 0, 31 };
constexpr Color COLOR_YELLOW = { 31, 31, 0 };
constexpr Color COLOR_MAGENTA = { 31, 0, 31 };
constexpr Color COLOR_AQUA = { 0, 31, 31 };
...
constexpr Color colorThemes[APP_MODE::COUNT] = {
    COLOR_AQUA,    // APP_MODE::PROGRAMMING_LBOE
    COLOR_GREEN,   // APP_MODE::PROGRAMMING_GBC
    COLOR_ORANGE,  // APP_MODE::PROGRAMMING_PICO_PROJECT
    COLOR_YELLOW,  // APP_MODE::KEYBINDS
    COLOR_MAGENTA, // APP_MODE::GAME_PHOTON_SMASH
};
...
struct App {
    ...
    u32 rainbowColorTimer;
    u8 rainbowColorUpdateRate;
    Color rainbowHSVColor;
};
...
static void toggle_light(u8 index) {
    rgbKeypad.set_color(index, app.photonSmash.color, rgbKeypad.get_brightness(index) == 0 ? 0.65f : 0);
}
...
static void default_selections() {
    for(i32 i = 0; i < APP_MODE::COUNT; ++i) {
        rgbKeypad.set_color(i, colorThemes[i], 0.075f);
    }

    rgbKeypad.set_color(app.mode, colorThemes[app.mode], 0.25f);

    rgbKeypad.set_color(7, COLOR_RED, 0.15f);
}
...
static void app_switch_mode(APP_MODE newMode) {
    ...
    switch(newMode) {
        ...
        case APP_MODE::KEYBINDS:
            ...
            for(i32 i = 8; i < 16; ++i) {
                rgbKeypad.set_color(i, colorThemes[newMode], 0.2f);
            }
            break;
        ...
        case APP_MODE::GAME_PHOTON_SMASH: {
            ...
            if(lvl < ARRAY_LENGTH(photonSmashPredefinedLevels)) {
                ...
                for(i32 i = 0, count = predefinedLevel->lightsCount; i < count; ++i) {
                    rgbKeypad.set_color(predefinedLevel->lights[i], app.photonSmash.color, 0.75f);
                }
                ...
                while(startWithLigjts-- > 0) {
                    ...
                    rgbKeypad.set_color(positions[r], app.photonSmash.color, 0.65f);
...
int main() {
    ...
    app.rainbowColorTimer = 0;
    app.rainbowColorUpdateRate = 1; // updates before changing color
    app.rainbowHSVColor = { 0, 31, 31 };
...
// In debug mode check all the predefined levels can be solved
    ...
    for(i32 i = 0; i < ARRAY_LENGTH(photonSmashPredefinedLevels); ++i) {
        ...
        if(!photon_smash_solvability_check(lights)) {
            ...
            rgbKeypad.set_color(i, COLOR_YELLOW, 1.0f);

            i -= 16;

            if(i > 0) {
                rgbKeypad.set_color(i / 16, COLOR_RED, 1.0f);
            }
    ...
    while(true) {
        ...
        if(app.updateTimer >= app.updateRate) {
            ...
            app.rainbowColorTimer += 1;

            if(app.rainbowColorTimer >= app.rainbowColorUpdateRate) {
                app.rainbowColorTimer -= app.rainbowColorUpdateRate;
                app.rainbowHSVColor.r = (app.rainbowHSVColor.r + 1) % 32;
            }
                        ...
                        switch(app.photonSmash.state) {
                            case PHOTON_SMASH_STATE::GAME:
                                if(app.photonSmash.rainbowLevel) {
                                    rgbKeypad.set_color(hsv_to_rgb(app.rainbowHSVColor));
                                }
                            ...
                            case PHOTON_SMASH_STATE::WIN_ANIMATION: {
                                ...
                                rgbKeypad.set_color(((app.photonSmash.animationTime / 250) & 1) ? COLOR_GREEN : COLOR_WHITE);
                            ...
                            case PHOTON_SMASH_STATE::UNSOLVABLE_ANIMATION: {
                                ...
                                rgbKeypad.set_color(((app.photonSmash.animationTime / 250) & 1) ? COLOR_RED : COLOR_YELLOW);
